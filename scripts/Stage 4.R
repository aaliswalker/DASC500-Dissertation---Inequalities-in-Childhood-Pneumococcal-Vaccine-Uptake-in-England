...
# Load main dataset
data_clean <- read.csv("COVER_All_Years_MERGED_WITH_IMD.csv")
...
quarterly_deprivation_all_scenarios <- map_dfr(ve_scenarios, function(scenario) {
  data_clean %>%
    ...
    mutate(
      ...
      p1 = PCV_12m_prop - PCV_24m_prop,         # Primary doses only
...
# Load England map
england_map <- st_read("Local_Authority_(Upper_Tier)_IMD_2019_(WGS84)/Local_Authority_(Upper_Tier)_IMD_2019_(WGS84).shp")
...
# Create quintile assignment map
quintile_map <- ggplot(imd_quintile_map_data) +
  geom_sf(aes(fill = as.factor(imd_quintile)), color = "white", linewidth = 0.2) +
  scale_fill_manual(
    name = "IMD Quintile",
    values = viridis::viridis(5),  # Changed from pastel_palette[1:5] to viridis
    labels = c("1 (Least Deprived)", "2", "3", "4", "5 (Most Deprived)")
  ) +
  labs(
    #subtitle = "Based on 2019 Index of Multiple Deprivation",
    #caption = "Each local authority is assigned to a deprivation quintile based on average IMD score"
  ) +
  theme_void() +
  theme(
...
if(length(bham_2plus1_12m) > 0) {
  ...
  p1_2plus1 <- (bham_2plus1_12m - bham_2plus1_24m) / 100
...
if(length(bham_1plus1_12m) > 0) {
  ...
  p1_1plus1 <- (bham_1plus1_12m - bham_1plus1_24m) / 100
